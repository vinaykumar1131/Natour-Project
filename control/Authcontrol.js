...
const userSignupEmail = require('../control/signupemial');
...
exports.restictto = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new Apperror('you have no permissiont to do this action', 401)
      );
    }
    next();
  };
...
exports.forgotpassword = catchasnc(async (req, res, next) => {
  ...
  try {
    ...
    await new sendmail(reset, resetURL).sendPasswordReset();
